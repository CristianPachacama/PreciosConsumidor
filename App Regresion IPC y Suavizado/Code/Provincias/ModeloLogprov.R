...
h=1
IPChist = read_excel("Data/provincial/ipc_ind_nac.xls",sheet = h,skip = 4)
dipc=dim(IPChist)[1]
IPChist = IPChist[-c(dipc,dipc-1),]
IPChist = as.data.frame(IPChist)
IPChist = IPChist[c(-1),]
dia2=as.Date("1/6/2018",format="%d/%m/%Y")
dia1=as.Date("1/1/2005",format="%d/%m/%Y")

fecha= seq(dia1,dia2,by= "month")
fecha= as.character(fecha,format="%d/%m/%y")
names(IPChist)=c("N.orden","ITEM",fecha)
